...
QJsonObject NOTAM::NOTAM::areaGeoJSON() const
{
    ...
    auto buildCircleFeature = [&](const QGeoCoordinate& center, double radiusNM) -> QJsonObject
    {
        ...
        double radiusM = radiusNM * 1852.0;
        double dLat = radiusM / 111320.0;
        double dLon = radiusM / (111320.0 * qCos(center.latitude() * pi / 180.0));

        QJsonArray ring;
        for (int i = 0; i <= nPoints; i++)
        {
            double angle = 2.0 * pi * i / nPoints;
            double lat = center.latitude() + dLat * qSin(angle);
            double lon = center.longitude() + dLon * qCos(angle);
            QJsonArray coord;
            coord.append(lon);
            coord.append(lat);
            ring.append(coord);
    ...
    {
        QString polyStr = polyMatch.captured(0);
        ...
        double latSpanNM = (maxLat - minLat) * 60.0;
        double lonSpanNM = (maxLon - minLon) * 60.0 * qCos((minLat + maxLat) / 2.0 * pi / 180.0);
